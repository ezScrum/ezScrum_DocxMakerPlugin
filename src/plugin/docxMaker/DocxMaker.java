...
public class DocxMaker {
	...
	public File getReleasePlanDocx(ReleasePlanObject releases, List<SprintPlanObject> sprints, HashMap<String, List<StoryObject>> storyMap, LinkedHashMap<Long, List<TaskObject>> taskMap,
	...
	private void addStoryInfo(MainDocumentPart mainDoc, List<StoryObject> storyList, LinkedHashMap<Long, List<TaskObject>> taskMap) throws JAXBException {
		...
		for (int i = 0; i < storySize; i++) {
			try {
				...
				tc.getContent().set(0, mainDoc.createParagraphOfText("Sprint Backlog Item #" + story.id));
				
				// add Name
				row = (Tr) table.getContent().get(1);
				element = (JAXBElement) row.getContent().get(0);
				tc = (Tc) element.getValue();
				tc.getContent().set(0, mainDoc.createParagraphOfText(story.name));
				...
				tc.getContent().set(0, mainDoc.createStyledParagraphOfText("Bold_Number", story.importance));
				...
				element = (JAXBElement) tr.getContent().get(0);
				tc = (Tc) element.getValue();
				tc.getContent().set(0, mainDoc.createParagraphOfText(story.notes));
				...
				tc = (Tc) element.getValue();
				setCenterProperty(tc);
				tc.getContent().set(0, mainDoc.createStyledParagraphOfText("Bold_Number", story.estimation));
				...
				element = (JAXBElement) tr.getContent().get(0);
				tc = (Tc) element.getValue();
				tc.getContent().set(0, mainDoc.createParagraphOfText(story.howToDemo));
		...
		for (int row = 0; row < rows; row++) {
			...
			for (int col = 0; col < cols; col++) {
				...
				if (col == 0) {	// task Id;
					...
					tc.getContent().add(mainDoc.createParagraphOfText(taskList.get(row).name));
					...
					tc.getContent().add(mainDoc.createParagraphOfText(taskList.get(row).handler));
				} else if (col == 4) {	// task Partners
					tc.getContent().add(mainDoc.createParagraphOfText(taskList.get(row).partners));
				} else if (col == 5) {	// task Notes
					tc.getContent().add(mainDoc.createParagraphOfText(taskList.get(row).notes));
				}
				tr.getContent().add(tc);
				TcPr tcPr = factory.createTcPr();
				tc.setTcPr(tcPr);
